# DMs individuales · Sesión 1 · 21/4/2026

**16 participantes + Alejo (soporte)** · cada mensaje adaptado al perfil.

---

## ⚙️ Antes de mandar · 2 reemplazos globales

| Placeholder | Qué poner |
|---|---|
| `<FECHA_VIERNES>` | Ej: *"viernes 25/4 a las 11:00"* |
| `<CALENDLY_LINK>` | Ej: `calendly.com/juan-estrada-itti/way-of-work` · o *"mandame 3 horarios y agendamos"* |

Los links **ya están embebidos**:
- Kit de instalación · `https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md`
- Repo del proyecto (workspace del viernes) · `https://github.com/juan-estrada-itti/taller-sdd-favoritos`
- Canal Slack · `#way-of-work` (ya existe)

---

## Cómo mandarlos

- Slack · **DM uno a uno** · no grupal
- Orden sugerido · técnicos primero (14:00-15:00) · no-técnicos después (15:00-16:00)
- **Juan Ma, Javier, Andrés** a la mañana del martes · son seniors, agenda llena
- Alejo primero que todo · necesita tiempo para prepararse

---

# GRUPO 1 · Técnicos seniors / roles especiales

## Alejo Bernal · Soporte

```
Alejo ·

Taller 1 cerrado · impecable como siempre tu mirada lateral
sobre el chat. Gracias por bancarla.

3 pedidos para esta semana y viernes (<FECHA_VIERNES>):

1 · Instalá el kit vos también · 30 min · así podés resolver
en vivo lo que te pregunten los demás:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

2 · Durante la semana · monitoreá Slack #way-of-work. Van a
preguntar cómo instalar · las respuestas están en:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/FAQ.md

   Si algo no está en FAQ · lo agregamos juntos · así crece.

3 · El viernes son 2 horas · mismos emoji signals:
   👀 atento · 🆘 ayuda · ✅ ok · 🔥 algo roto

Los 4 no-técnicos del grupo son Ana · Juan Pa · Leo · Juan Ma.
Si responden trabados · me etiquetás o respondés con paciencia ·
ellos son el caso de adopción más importante · si los perdemos
a ellos, se baja el nivel del equipo.

Gracias máquina ·
Juan
```

---

## Sergio Amarilla · Full stack

```
Sergio ·

Gracias por el taller · tu experiencia con Opus 4.6 y Cursor
le dio temple al grupo · los demás vieron que sí se puede.

Dos cosas antes del próximo viernes (<FECHA_VIERNES>):

1 · Instalá el kit · 10 min para vos · ya tenés todo andando:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

   O directo 1 comando:
   curl -fsSL https://raw.githubusercontent.com/juan-estrada-itti/way-of-work-tools/main/kit/install-kit.sh | bash

2 · Pensá 1 idea greenfield · real o inventada · la traés el viernes.

PS · si ves algo del script que mejoraría (mirando que lo uses
con Cursor también), PR al repo y lo revisamos.

Cualquier cosa ·
Juan
```

---

## Javier Durán · VP sponsor · Pareja con Sergio

```
Javier ·

Gracias por bancar el taller hoy · la presencia de VP cambia
el tono · sostiene el compromiso del resto del equipo.

Dos cosas antes del viernes (<FECHA_VIERNES>):

1 · El kit · 30 min máximo · tu setup con Docker que me pasaste
me sirvió de referencia para el install-kit.sh · mirá el hilo:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

   Si querés hacerlo por Docker (tu script original), avisame
   y hacemos una versión dedicada en tools/.

2 · El viernes son 2 horas · ejercicio real en parejas · tu
pareja es Sergio otra vez · fase PRD. Si podés estar 100%
presente es clave para la señal al resto.

Si el timing no va, me avisás · ajustamos.

Gracias ·
Juan
```

---

## Fede Correa · AI Engineer · Pareja con Andrés

```
Fede ·

Gracias por el taller · tu mirada de AI eng dio contexto técnico
que el resto no hubiera tenido de otra forma.

Antes del viernes (<FECHA_VIERNES>):

1 · Kit instalado · 10 min:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

   Fijate especialmente en skills-overlays/ · son las v2 que
   agregan schema de dependencias al frontmatter · si te
   convencen, las PR'eas al repo tech_emergentes_skills.

2 · 1 idea greenfield para el relay · puede ser algo de tu
stack favorito (LangGraph · agentes · lo que te pinte).

Una cosa que me quedó rondando · el grafo de dependencias entre
skills está declarativo ahora · si sabés de alguna forma más
elegante de representarlo (tipo pipeline-as-code), hablemos.

Nos vemos viernes ·
Juan
```

---

## Andrés López · Head de tech emergentes · Pareja con Fede

```
Andrés ·

Gracias por venir · que estés tan presente en la operativa le
da credibilidad al way of work ante el resto del equipo.

Una sola cosa antes del viernes (<FECHA_VIERNES>):

1 · Kit instalado · 30 min:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

El viernes · relay de 8 parejas · tu pareja es Fede · fase RFC.
Son 2 horas. Tu presencia como head importa tanto o más que la
de cualquier otro.

Una nota · mientras probás el kit, si notás algo que no
cierra con la visión que tenés de tech emergentes, avisame.
Este sistema no es mío · es del equipo · se ajusta.

Cualquier cosa ·
Juan
```

---

# GRUPO 2 · Técnicos estándar

## Hugo Escobar · Frontend (Cursor) · Pareja con Juan Pa

```
Hugo ·

Gracias por venir. Que trabajes full en Cursor nos ayuda a
recordar que el way of work tiene que funcionar en las 3 IDEs ·
no atarnos a Claude Code pelado.

Antes del próximo viernes (<FECHA_VIERNES>):

1 · Instalá el kit · 10 min · 1 comando:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

   Como usás Cursor, algunas skills se cargan distinto (Rules
   en vez de SKILL.md). El kit tiene una sección sobre eso ·
   si algo no queda claro, Slack #way-of-work.

2 · Pensá 1 idea greenfield · para el ejercicio del viernes.

PS · tu pareja es Juan Pa · fase Contratos (api-contract.md).
Vas a tirar FE, Juan Pa BE/negocio. Él es menos técnico ·
apoyalo si se traba.

Nos vemos ·
Juan
```

---

## Luciano Pérez · Tech · Pareja con Ana María

```
Luciano ·

Gracias por venir al taller.

Antes del viernes (<FECHA_VIERNES>):

1 · Kit · 15 min:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

2 · 1 idea greenfield para el relay.

Una cosa · tu pareja el viernes es Ana María (cultura) · fase
Historias INVEST. Ella trae la mirada de usuario y vos el
rigor técnico · es la pareja más balanceada del grupo.
Cuando ella proponga algo "del usuario", traducilo a historia
INVEST en voz alta · se nota mucho y Ana aprende rápido.

Nos vemos ·
Juan
```

---

## Nicolás Canese · Tech · Pareja con Nicolás Cóppola

```
Nico Canese ·

Gracias por venir. El tándem con Cóppola es de los más fuertes
del grupo en mirada técnica.

Antes del viernes (<FECHA_VIERNES>):

1 · Kit · 15 min · directo:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

2 · 1 idea greenfield.

PS · fase que les toca el viernes con Cóppola es Tasks · plan
ejecutable por historia + DAG. Son los que cierran antes del
código · mucha responsabilidad técnica · ustedes pueden.

Nos vemos ·
Juan
```

---

## Nicolás Cóppola · Tech · Pareja con Nicolás Canese

```
Nico Cóppola ·

Gracias por venir. Con Canese tienen buena onda técnica.

Antes del viernes (<FECHA_VIERNES>):

1 · Kit · 15 min:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

2 · 1 idea greenfield.

PS · tu pareja es Canese · fase Tasks. Lo que generan ustedes
es el input directo para que el código se escriba (con Claude
o con manos) · si ustedes definen mal las subtasks, todo lo
demás falla. Ojo con eso · pero tienen la cabeza para bancarla.

Nos vemos ·
Juan
```

---

## German Insuasty · Tech · Pareja con Fernando

```
German ·

Gracias por venir al taller.

Antes del viernes (<FECHA_VIERNES>):

1 · Kit instalado · 15 min:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

2 · Pensá 1 idea greenfield · para el relay del viernes.

PS · tu pareja es Fernando López · fase Code Review.
Son los últimos de la cadena · validan que el código cumpla
con todo lo que las otras parejas construyeron. El rol más
político-técnico de todos · bancala con criterio.

Nos vemos ·
Juan
```

---

## Fernando López · Tech · Pareja con German

```
Fernando ·

Gracias por venir.

Antes del viernes (<FECHA_VIERNES>):

1 · Kit · 15 min:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

2 · 1 idea greenfield.

PS · el viernes con German hacen Code Review · última fase del
relay. Van a recibir código (que quizá tenga errores adrede
para aprender) · ustedes lo validan contra PRD, RFC, contrato,
ADRs. Es trabajo de juicio más que de código.

Nos vemos ·
Juan
```

---

## Stephany Rivera · Pareja con Juan Ma

```
Stephany ·

Gracias por venir al taller.

Antes del viernes (<FECHA_VIERNES>):

1 · Kit instalado · 15-30 min según experiencia terminal:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

   Si preferís hacerlo acompañada · agendame acá:
   <CALENDLY_LINK>

2 · 1 idea de feature greenfield para el ejercicio.

PS · tu pareja es Juan Ma · fase Discovery. Ustedes abren el
relay · dan el tono al resto. Juan Ma ya conoce /office-hours
(él ayudó a construir /create-prd) · vos aportá mirada nueva
para no caernos en "lo que ya sabemos".

Nos vemos ·
Juan
```

---

## Carlos Espinosa · Pareja con Leo

```
Carlos ·

Gracias por venir.

Antes del viernes (<FECHA_VIERNES>):

1 · Kit · 15-30 min:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

   Si querés hacerlo juntos en 20 min:
   <CALENDLY_LINK>

2 · 1 idea greenfield.

PS · tu pareja es Leo · fase Historias INVEST. Leo es PM de ulab ·
trae visión producto · vos aportá detalle técnico de cómo se
construye · complementan bien.

Nos vemos ·
Juan
```

---

# GRUPO 3 · No-técnicos · tono más humano

## Juan Manuel Salazar · PM · Pareja con Stephany

```
Juan Ma ·

Gracias por el taller · tu experiencia haciendo /create-prd
fue importante para el grupo · vieron que esto no es teoría ·
alguien de adentro ya lo usó.

Antes del viernes (<FECHA_VIERNES>):

1 · Instalá el kit · 20 min · vos ya tenés ambiente de Claude ·
te toca solo agregar las skills nuevas:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

   Si algo no anda · directo escribime · no pierdas tiempo.

2 · Pensá 1 idea greenfield · algo real que te gustaría
construir · si es para academy o ulab mejor (aunque sea sub-feature).

PS · tu pareja es Stephany · fase Discovery. Ustedes abren el
relay · el tono que pongan lo sigue el resto de la mañana.

Cualquier cosa ·
Juan
```

---

## Ana María Villegas · Cultura / Cambio Organizacional · Pareja con Luciano

```
Ana ·

Gracias por venir al taller · tu mirada de cultura nos baja a
tierra cuando los técnicos nos perdemos en el "cómo" y olvidamos
el "para qué". Importante que sigas adentro.

Antes del viernes (<FECHA_VIERNES>) una sola cosa · dejar la
compu lista. 2 opciones, elegís:

Opción 1 · lo hacés sola · guía paso a paso en castellano:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

Opción 2 · 20 min de videollamada conmigo · lo hacemos juntos:
<CALENDLY_LINK>

Si algún término te suena raro · mandame por acá la palabra ·
te lo traduzco sin jerga.

¿Y si no llegás? Venís igual · el viernes trabajás en pareja
con Luciano (tech) · tu rol no cambia.

Una cosa más · si alguna vez sentís que la conversación se
vuelve muy técnica · preguntá igual · probablemente los demás
también están perdidos y no se animan. Tu permiso para bajar
la temperatura es valioso para el grupo entero.

Un abrazo ·
Juan
```

---

## Juan Pablo Fernández · Líder de negocio · Pareja con Hugo

```
Juan Pa ·

Gracias por participar del taller. La mirada de negocio es lo
que evita que el equipo técnico se vaya por la tangente ·
crucial tenerla en la mesa.

Setup para el viernes (<FECHA_VIERNES>) · 30 min · misma compu
que usaste hoy:
https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

Si algo no anda · ping directo · lo resolvemos en 10 min · no
quiero que pierdas tiempo en cuestiones técnicas.

Si preferís hacerlo juntos:
<CALENDLY_LINK>

El viernes tu pareja es Hugo (FE dev) · fase Contratos API ·
ustedes cierran el puente entre producto y técnica · tu lado
es "qué valor tiene que entregar" · el de Hugo "cómo se
materializa". Balance perfecto.

Pensá 1 idea greenfield (real · de tu iniciativa) · la
usamos como input.

Cualquier cosa ·
Juan
```

---

## Leonardo Villa · PM ulab · Pareja con Carlos

```
Leo ·

Gracias por venir al taller. Como PM de ulab tu mirada producto
es clave para que este way of work sirva al equipo real ·
no quede en teoría de arquitectos.

Antes del próximo viernes (<FECHA_VIERNES>) una sola cosa ·
dejar la compu lista para el ejercicio en vivo.

2 opciones, elegís:

Opción 1 · lo hacés solo · 30-40 min
   Guía paso a paso (tiene comandos copy-paste):
   https://github.com/juan-estrada-itti/way-of-work-tools/blob/main/kit/INSTALL.md

   Y cloná el repo del viernes (workspace del relay · 2 min):
   https://github.com/juan-estrada-itti/taller-sdd-favoritos

   Si te trabás · Slack #way-of-work · Alejo y yo respondemos.

Opción 2 · 20 min de videollamada conmigo
   Si la terminal te saca · mejor esto · lo hacemos juntos:
   <CALENDLY_LINK>

¿Y si no llegás? Venís igual · el viernes trabajás con Carlos ·
tu rol no cambia.

Una cosa más · pensá 1 idea de feature de tu backlog de ulab ·
real. La usamos como input del ejercicio · así hablamos de algo
tuyo · no de algo abstracto.

PS · tu fase el viernes con Carlos es Historias INVEST · es lo
que hacés todos los días · vas a estar cómodo.

Cualquier cosa ·
Juan
```

---

# Mensaje grupal · después de los DMs individuales

Después de mandar los 17 DMs · mandás esto al canal `#way-of-work`:

```
Equipo ·

Taller 1 cerrado · tengo buenos aprendizajes que compartiré
la semana que viene.

Material completo + kit de instalación en:
https://github.com/juan-estrada-itti/way-of-work-tools

Antes del próximo viernes <FECHA_VIERNES>, 2 tareas async:

  • Instalar el kit en tu compu (30-40 min)
  • Traer 1 idea de feature greenfield para el ejercicio

Soporte durante la semana en este canal · respondemos entre
yo y Alejo.

Feliz de cómo arrancamos · nos vemos el viernes ·
```

---

# Resumen · tiempos y orden de envío

| Día · hora | A quién |
|---|---|
| **Hoy 13:00** | Alejo (primero · necesita tiempo para prepararse) |
| **Hoy 14:00** | Sergio · Fede · Hugo · Luciano · Nicolás Canese · Nicolás Cóppola · German · Fernando · Stephany · Carlos (técnicos) |
| **Hoy 15:00** | Juan Ma · Ana · Juan Pa · Leo (no-técnicos · con más tiempo de respuesta por si responden trabados) |
| **Mañana 09:00** | Javier · Andrés (seniors · agenda llena · mañana es mejor) |
| **Hoy 16:00** | Mensaje grupal al canal `#way-of-work` |
| **Martes 11:00** | Nudge general en el canal · *"¿cómo va la instalación?"* |
| **Miércoles 15:00** | 1-on-1s con no-técnicos que no pudieron |
| **Jueves 18:00** | Último recordatorio · *"los que faltan ajustan mañana a la mañana"* |

---

# Checklist antes de apretar send

Por cada mensaje:
- [ ] Reemplazaste `<FECHA_VIERNES>` con la fecha+hora reales
- [ ] Reemplazaste `<CALENDLY_LINK>` donde corresponde (no-técnicos + Stephany + Carlos)
- [ ] Saludás con el nombre correcto (ojo con los 2 Nicolás)
- [ ] Verificaste que el DM es individual, no grupal

---

**Última actualización** · 2026-04-21 · Juan Estrada
